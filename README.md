...
#### 2. 観光ルート作成
```
浅草寺周辺の観光スポット（レストラン、神社、公園）と歩行者道路のデータが欲しい
```
→ 観光客向けの歩行ルートや見どころマップを作成

#### 3. 災害時避難計画
```
学校周辺の避難に使える道路、公園、公共施設のデータを収集したい
```
→ 避難経路や避難場所の最適化に活用

#### 4. 交通インフラ調査
```
品川駅周辺の鉄道、道路、バス停のデータで交通アクセスを分析したい
```
→ 交通利便性の評価や都市計画に活用
...
#### QGIS での地図作成
```
東京都心部の建物データをGeoJSON形式でファイル保存して、QGISで3D地図を作成したい
```

#### Web地図アプリ開発
```
Leafletで使用する地域のPOIデータ（レストラン、病院、学校）をJSON形式で取得してください
```

#### データ分析・機械学習
```
複数エリアの道路密度データを収集して、都市発展パターンの分析に使いたい
```
